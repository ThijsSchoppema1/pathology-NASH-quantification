# An lightning module to train in a MIL schema, however by only taking the top N best performing patches
...
class LitModelTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        losses = self.loss(y_hat, targets.float())
        if self.selection_size is not None:
            loss = []
            for i in range(len(losses) // self.sample_size):
                start = self.sample_size * i
                end = start + self.sample_size
                tk, _ = torch.topk(losses[start:end], self.selection_size)
                loss.append(tk)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.selection_size is not None:
            ...
            for i in range(len(losses) // self.sample_size):
                ...
                tk, idx = torch.topk(losses[start:end], self.selection_size)
                idxs.append(idx)
                loss.append(tk)
            y_hat = y_hat[torch.cat(idxs)]
            targets = targets[torch.cat(idxs)]
            loss = torch.mean(torch.stack(loss))
        else:
            loss = torch.mean(losses)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.selection_size is not None and False:
            loss = []
            idxs = []
            for i in range(len(losses) // self.sample_size + 1):
                start = self.sample_size * i
                end = start + self.sample_size
                tk, idx = torch.topk(losses[start:end], self.selection_size)
                idxs.append(idx)
